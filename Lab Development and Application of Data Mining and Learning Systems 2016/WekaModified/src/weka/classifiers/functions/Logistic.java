...
public class Logistic extends Classifier 
  ...
  /**
   * Builds the classifier by overwriting parameters
   *
   * @param coefficients paramaters to be written to build classifier
   * @param test the test data to be used for updating internal
   * variables of the classifier
   * @throws Exception if the classifier could not be built successfully
   */
  public void buildClassifier(double[][] coefficients, Instances test) throws Exception
  {
	  	// can classifier handle the data?
	    getCapabilities().testWithFail(test);
	    ...
	    m_ReplaceMissingValues.setInputFormat(test);
	    test = Filter.useFilter(test, m_ReplaceMissingValues);

	    // Remove useless attributes
	    m_AttFilter = new RemoveUseless();
	    m_AttFilter.setInputFormat(test);
	    test = Filter.useFilter(test, m_AttFilter);
		
	    // Transform attributes
	    m_NominalToBinary = new NominalToBinary();
	    m_NominalToBinary.setInputFormat(test);
	    test = Filter.useFilter(test, m_NominalToBinary);
	    
	    // Save the structure for printing the model
	    m_structure = new Instances(test, 0);
		
	    // Extract data
	    m_ClassIndex = test.classIndex();
	    m_NumClasses = test.numClasses();

	    int nK = m_NumClasses - 1;                     // Only K-1 class labels needed 
	    int nR = m_NumPredictors = test.numAttributes() - 1;
	    int nC = test.numInstances();
	    ...
	    for(int i = 0; i < nR + 1; i ++)
	    {
	    	for (int j = 0; j < nK; j ++)
	    	{
	    		m_Par[i][j] = coefficients[i][j];
	    	} // for
	    } // for
	    
	    //nullify the data, we do not have any training data
	    m_Data = null;
  } // buildClassifier
