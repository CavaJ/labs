...
public class SMO 
  ...
  /**
   * Method for building the classifier from given weights.
   * 
   * @param bias bias for classifier
   * @param weights given weights
   * @param insts the set of test instances
   * @throws Exception if the classifier can't be built successfully
   */
  public void buildClassifier(double bias, double[] weights, Instances insts) throws Exception {
	    ...
	    //m_KernelIsLinear is always true
	    //System.out.println("m_KernelIsLinear is " + m_KernelIsLinear);
		...
		//numClasses() will be 2 always
		m_classifiers = new BinarySMO[insts.numClasses()][insts.numClasses()];
		
		//for concept learning we only need index [0][1], so
		m_classifiers[0][1] = new BinarySMO();
		m_classifiers[0][1].setKernel(Kernel.makeCopy(getKernel()));
		
		//set sparse weights
		m_classifiers[0][1].m_sparseWeights = new double[weights.length];
		
		//iteratively copy values
		for (int index = 0; index < weights.length; index ++)
		{
			m_classifiers[0][1].m_sparseWeights[index] = weights[index];
		} // for
		
		//set bias
		m_classifiers[0][1].m_b = bias;
		
		//set sparse indices
		m_classifiers[0][1].m_sparseIndices = new int[weights.length];
		for (int index = 0; index < weights.length; index ++)
		{
			//it will be [1, 2, 3, 4, 5.....]
			m_classifiers[0][1].m_sparseIndices[index] = index + 1;
		} // for
		
		//set the sum of weights
		// Store the sum of weights
		m_classifiers[0][1].m_sumOfWeights = insts.sumOfWeights();
		
		//we will not fit logistic here
		setBuildLogisticModels(false);
		m_classifiers[0][1].m_logistic = null;
		
		// Initialize some variables
		m_classifiers[0][1].m_bUp = -1;
		m_classifiers[0][1].m_bLow = 1;
		m_classifiers[0][1].m_data = null;
		m_classifiers[0][1].m_errors = null;
		m_classifiers[0][1].m_I0 = null;
		m_classifiers[0][1].m_I1 = null;
		m_classifiers[0][1].m_I2 = null;
		m_classifiers[0][1].m_I3 = null;
		m_classifiers[0][1].m_I4 = null;
		
		// Set class values
		m_classifiers[0][1].m_class = new double[insts.numInstances()];
		m_classifiers[0][1].m_iUp = -1;
		m_classifiers[0][1].m_iLow = -1;
		for (int i = 0; i < m_classifiers[0][1].m_class.length; i++) 
		{
			if ((int) insts.instance(i).classValue() == 0) { // label can be 0
				m_classifiers[0][1].m_class[i] = -1;
				m_classifiers[0][1].m_iLow = i;
			} else if ((int) insts.instance(i).classValue() == 1) { // or label can be 1
				m_classifiers[0][1].m_class[i] = 1;
				m_classifiers[0][1].m_iUp = i;
			} else {
				throw new Exception("This should never happen!");
			}
		} // for
		...
		// Check whether one or both classes are missing
		if ((m_classifiers[0][1].m_iUp == -1)
				|| (m_classifiers[0][1].m_iLow == -1)) 
		{
			if (m_classifiers[0][1].m_iUp != -1) 
			{
				m_classifiers[0][1].m_b = -1;
			} // if 
			else if (m_classifiers[0][1].m_iLow != -1) 
			{
				m_classifiers[0][1].m_b = 1;
			} // else if 
			else 
			{
				m_classifiers[0][1].m_class = null;
				return;
			} // else
		...
		//we do not errors and intermediate calculations
		m_classifiers[0][1].m_errors = null;
		m_classifiers[0][1].m_I0 
			= m_classifiers[0][1].m_I1 
			= m_classifiers[0][1].m_I2 
			= m_classifiers[0][1].m_I3 
			= m_classifiers[0][1].m_I4 
			= null;
		...
		if (m_KernelIsLinear) {
			...
			// We don't need to store the set of support vectors
			m_classifiers[0][1].m_supportVectors = null;

			// We don't need to store the class values either
			m_classifiers[0][1].m_class = null;
			
			// Clean out weight vector, because we already have sparse weights and indices
			m_classifiers[0][1].m_weights = null;
			
			// We don't need the alphas in the linear case
			m_classifiers[0][1].m_alpha = null;
		} // if
