...
public class ParallelRadonMachine 
{
	...
	public static String PRM_OUTPUT_FILE = "PRM_Models";
	public static String SPARK_OUTPUT_FILE = "Spark_Models";
	
	public static void run(String master, String hdfsInputFilePath, String localInputFilePath, 
			String outputDirectory, int noOfIterations_H, 
			BaseLearner baseLearner, SparkLearner sparkLearner, 
			boolean discard, String hadoopHomeDir, boolean isLocalMode)
	{	
		...
		//get file name here
		File file = new File(localInputFilePath);
		...
		//for generating unique file name of PRM models
		PRM_OUTPUT_FILE += "_" + baseLearner.toSimpleStirng();
		PRM_OUTPUT_FILE += "_" + datasetName;
		PRM_OUTPUT_FILE += "_H_" + noOfIterations_H + ".txt";
		
		//for generating unique spark file name
		SPARK_OUTPUT_FILE += "_" + sparkLearner;
		SPARK_OUTPUT_FILE += "_" + datasetName;
		SPARK_OUTPUT_FILE += "_H_" + noOfIterations_H + ".txt";
				
		//determine number of features of one data instance beforehand
		Integer numFeatures = getNumFeatures(localInputFilePath);
		...
		{					
			...
			configuration.set("spark.executor.memory", "2g"); //1GB default
			//noOfProcessors_C workers
	      	configuration.set("spark.executor.instances", "" + noOfProcessors_C);
		} // if
		else
		{
			//FOR LOCAL MODE
			//noOfProcessors_C threads
			configuration.setMaster("local[" + noOfProcessors_C + "]");
		} // else				
		...
		/*
		crossValidateForSparkLearner(hdfsInputFilePath, 
					localInputFilePath, outputDirectory, 
		...
		cvSplits[1].repartition(requiredPartitionCount);
	    ...
	    System.out.println("Data partitions: " + cvSplits[1].getNumPartitions());
			...
			String resultString = "\n****** FOLD #" + (j + 1) + " ******\n";
	  		...
	  		JavaRDD<Vector> models = trainingData.mapPartitions(
				new FlatMapFunction<Iterator<LabeledPoint>, Vector>() 
				{
					...
					{	
						// default options
						String defaultOptions = "-C 1 -N 2 -L 1.0e-3 -P 1.0e-12 -V -1 -W 1"; // -N 2
		...
		//get the end time of the fold
	  	...
	  	//RESET file names
		PRM_OUTPUT_FILE = "PRM_Models";
		SPARK_OUTPUT_FILE = "Spark_Models";
	 ...
	 * */
	private static void crossValidateForSparkLearner(String hdfsInputFilePath, 
			String localInputFilePath,
	...
	{	
	    ...
	    //and average runtime of the Spark Parallel Algorithm
	  	for (int j = 0; j < numFolds; j ++)
	  	{
			...
			{
				//train and build sparkModel from best parameters
				LogisticRegressionModel sparkModel = new LogisticRegressionWithLBFGS().run(trainingData.rdd());
